...
public final class CommonUtilities {
     ...
     */
    public static String popunReadMessages(Context context) {
        SharedPreferences sp =
                context.getSharedPreferences("messages", Context.MODE_PRIVATE);
        String last = sp.getString("unRead", null);
        sp.edit().putString("unRead", null).commit();
        return last;
    }
    ...
    public static int storeMessage(Context context, String message) {
        ...
        String oldMsg=sp.getString("unRead", "")+"\n";
        sp.edit().putInt("messages", messages).putString("unRead", oldMsg+message).commit();
        return messages;
    }
